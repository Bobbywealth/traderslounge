        ...
        ok = self.bot.dispatch_event(
            {
                ...
                "pair": "BTCUSD",
                "timeframe": "4h",
                "payload": {"direction": "BUY", "entry": 67500, "stop": 66200, "targets": [69000]},
            },
            prefs,
        )
        self.assertTrue(ok)
...
class TelegramBotFormatTest(unittest.TestCase):
    def test_format_event_renders_required_fields(self):
        ...
        self.assertIn("67500", text)
        self.assertIn("69000", text)

    def test_format_event_critical_severity(self):
        text = TelegramBot.format_event(
            {
                "title": "News risk",
