...
from scanner.persistence import NullRepository
...
def _make_scanner(pairs, snapshots_by_pair, news=None):
    ...
    return scanner, sink, client, repo
...
class TestScheduler(unittest.TestCase):
    ...
    def test_emits_to_sink_when_above_threshold(self):
        ...
        scanner, sink, _, repo = _make_scanner(["XAUUSD"], {"XAUUSD": buy_snap})
        scanner.scan_once()
        ...
        self.assertEqual(repo.saved[0].pair, "XAUUSD")
    ...
    def test_data_error_records_in_result_and_skips_sink(self):
        ...
        self.assertEqual(len(repo.saved), 0)  # nothing to persist on error
    ...
    def test_news_blackout_skips_pair(self):
        ...
        scanner = Scanner(config=cfg, client=client, news=nf, sink=sink,
                          clock=FakeClock(clock_now), repository=repo)
        results = scanner.scan_once()
