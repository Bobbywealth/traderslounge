        ...
        <div className="space-y-6">
          <div className="relative overflow-hidden rounded-[24px] border border-violet-400/15 bg-[#090d18] bg-gradient-to-br from-violet-500/10 to-cyan-500/[0.04] p-6">
            ...
            {bestSignal && cryptoAnalysis ? <>
              <div className="mt-5 flex items-start justify-between"><div><div className="flex items-center gap-2"><div className="text-3xl font-black">{bestSignal.pair}</div><span className={`rounded-md px-2 py-1 text-[9px] font-black ${cryptoAnalysis.direction === 'BUY' ? 'bg-emerald-400/10 text-emerald-300' : cryptoAnalysis.direction === 'SELL' ? 'bg-rose-400/10 text-rose-300' : 'bg-slate-400/10 text-slate-400'}`}>{planReady ? cryptoAnalysis.direction : tradePlan?.status || 'WAIT'}</span></div><div className="mt-1 text-sm capitalize text-slate-400">{cryptoAnalysis.scenarios.primary}</div></div><div className="rounded-2xl border border-cyan-400/20 bg-cyan-400/10 px-4 py-3 text-center"><div className="text-2xl font-black text-cyan-300">{cryptoAnalysis.total_score}</div><div className="text-[8px] font-bold tracking-widest text-cyan-500">V2 / 100</div></div></div>
