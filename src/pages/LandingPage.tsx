...
const LandingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#05070d] cx-text-strong selection:bg-violet-500/40">
      ...
      <nav className="fixed inset-x-0 top-0 z-50 border-b cx-border bg-[#05070d]/75 backdrop-blur-2xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 lg:px-8">
          <a href="#top" aria-label="ConfluenceX home"><ConfluenceXLogo size="md" /></a>
          <div className="hidden items-center gap-8 text-sm font-medium cx-text-muted md:flex">
          ...
          <div className="hidden items-center gap-3 md:flex">
            <button onClick={() => openAuth('login')} className="rounded-xl px-4 py-2.5 text-sm font-semibold cx-text-muted transition hover:cx-bg-card-hover hover:cx-text-strong">Sign in</button>
          ...
          <button
            ...
            className="flex min-h-[44px] min-w-[44px] items-center justify-center rounded-xl border cx-border-strong p-2.5 md:hidden"
        ...
        {mobileMenuOpen && (
          <div
            ...
            className="border-t cx-border-strong bg-[#080b14] px-5 py-5 md:hidden"
          >
            <div className="flex flex-col gap-2 cx-text-muted">
              ...
              <button onClick={() => openAuth('login')} className="flex min-h-[44px] items-center rounded-xl border cx-border-strong px-3">Sign in</button>
            </div>
          </div>
        )}
      </nav>
      ...
      <main className="relative z-10">
        <section id="top" className="relative px-5 pb-20 pt-36 lg:pb-28 lg:pt-44">
          ...
          <div className="mx-auto max-w-7xl">
            ...
            <div className="relative mx-auto mt-16 max-w-6xl [perspective:1600px]">
              ...
              <div className="relative overflow-hidden rounded-[24px] border border-white/15 cx-bg-card/95 shadow-2xl shadow-black/60 lg:[transform:rotateX(2deg)]">
                <div className="flex h-12 items-center justify-between border-b cx-border px-4">
                  ...
                  <div className="rounded-md border cx-border bg-white/[0.03] px-4 py-1 text-[10px] font-semibold tracking-widest cx-text-faint">CONFLUENCEX / LIVE WORKSPACE</div>
                ...
                <div className="grid min-h-[430px] lg:grid-cols-[1fr_280px]">
                  <div className="relative overflow-hidden border-b cx-border p-5 lg:border-b-0 lg:border-r">
                    <div className="mb-5 flex items-center justify-between"><div><div className="text-sm font-black">EURUSD <span className="ml-2 text-xs font-medium cx-text-faint">15m</span></div><div className="mt-1 text-2xl font-black text-cyan-300">1.0821 <span className="text-xs text-emerald-400">+0.02%</span></div></div><div className="rounded-lg border border-cyan-400/20 bg-cyan-400/5 px-3 py-2 text-xs text-cyan-300">ADR 45% used</div></div>
                    <div className="relative h-72 overflow-hidden rounded-xl border cx-border bg-[#060912] chart-grid">
                  ...
                  <div className="p-4"><div className="mb-3 flex items-center justify-between"><span className="text-xs font-black tracking-widest cx-text-faint">LIVE SIGNALS</span><Activity className="h-4 w-4 text-cyan-300" /></div><div className="space-y-3">{signals.map((signal, index) => <div key={signal.pair} className="signal-card rounded-xl border cx-border cx-bg-card p-3" style={{animationDelay: `${index * 220}ms`}}><div className="flex items-center justify-between"><div className="font-black">{signal.pair} <span className="text-[10px] font-medium cx-text-faint">{signal.tf}</span></div><span className={`rounded px-2 py-1 text-[9px] font-black ${signal.side === 'LONG' ? 'bg-emerald-400/10 text-emerald-300' : 'bg-rose-400/10 text-rose-300'}`}>{signal.side}</span></div><div className="mt-3 flex items-end justify-between"><span className="text-xs cx-text-faint">{signal.price}</span><div className="text-right"><div className="text-lg font-black text-cyan-300">{signal.score}</div><div className="text-[8px] tracking-wider cx-text-faint">SCORE</div></div></div></div>)}</div></div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="border-y cx-border bg-white/[0.02] py-3"><div className="ticker-track flex w-max gap-12 whitespace-nowrap text-[11px] font-bold tracking-[0.15em] cx-text-faint">{[...ticker,...ticker,...ticker].map((item,index) => <span key={`${item}-${index}`} className={item.includes('+') ? 'text-emerald-400/80' : item.includes('-') ? 'text-rose-400/80' : ''}>{item}</span>)}</div></div>

        <section id="platform" className="px-5 py-28"><div className="mx-auto max-w-7xl"><div className="max-w-3xl"><div className="mb-4 text-xs font-black tracking-[0.22em] text-cyan-300">BUILT FOR DECISIVE TRADERS</div><h2 className="text-4xl font-black tracking-[-0.04em] sm:text-6xl">Every signal means more<br/><span className="cx-text-faint">when the evidence converges.</span></h2><p className="mt-5 max-w-2xl text-lg leading-relaxed cx-text-muted">ConfluenceX replaces tab chaos with one focused system for discovery, confirmation, execution planning, and review.</p></div><div className="mt-14 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{features.map(({icon:Icon,...feature},index) => <article key={feature.title} className="feature-card group relative overflow-hidden rounded-2xl border cx-border cx-bg-card p-6 transition duration-500 hover:-translate-y-2 hover:border-cyan-400/25 hover:cx-bg-card-hover" style={{animationDelay:`${index*100}ms`}}><div className="absolute right-0 top-0 h-28 w-28 rounded-full bg-violet-500/0 blur-3xl transition group-hover:bg-violet-500/15"/><div className="mb-8 flex h-12 w-12 items-center justify-center rounded-xl border cx-border-strong bg-gradient-to-br from-cyan-400/15 to-violet-500/15 text-cyan-300"><Icon className="h-5 w-5"/></div><div className="text-[10px] font-black tracking-[0.2em] cx-text-faint">{feature.eyebrow}</div><h3 className="mt-3 text-xl font-black tracking-tight">{feature.title}</h3><p className="mt-3 leading-relaxed cx-text-muted">{feature.description}</p><ChevronRight className="mt-6 h-5 w-5 cx-text-faint transition group-hover:translate-x-2 group-hover:text-cyan-300"/></article>)}</div></div></section>

        <section id="workflow" className="border-y cx-border bg-[#080b14] px-5 py-28"><div className="mx-auto grid max-w-7xl gap-16 lg:grid-cols-[.8fr_1.2fr] lg:items-center"><div><div className="text-xs font-black tracking-[0.22em] text-violet-300">THE CONFLUENCEX LOOP</div><h2 className="mt-4 text-4xl font-black tracking-[-0.04em] sm:text-6xl">A workflow that keeps emotion out.</h2><p className="mt-6 text-lg leading-relaxed cx-text-muted">Move from market discovery to structured review without losing the reason behind the trade.</p><button onClick={() => openAuth('signup')} className="mt-8 flex items-center gap-2 font-black text-cyan-300 transition hover:gap-4">Build your process <ArrowRight className="h-4 w-4"/></button></div><div className="relative"><div className="absolute left-6 top-8 h-[calc(100%-4rem)] w-px bg-gradient-to-b from-cyan-400 via-violet-500 to-fuchsia-500"/>{[['01','SCAN','Surface live opportunities across markets.'],['02','CONFIRM','Validate structure, harmonics, ADR, and score.'],['03','PLAN','Define invalidation and risk before execution.'],['04','REVIEW','Journal the outcome and sharpen the edge.']].map(([number,title,copy])=><div key={number} className="group relative mb-4 flex gap-5 rounded-2xl border cx-border cx-bg-card p-5 transition hover:translate-x-2 hover:border-violet-400/25"><div className="relative z-10 flex h-12 w-12 shrink-0 items-center justify-center rounded-xl border cx-border-strong cx-bg-elev text-xs font-black text-cyan-300">{number}</div><div><div className="text-xs font-black tracking-[0.2em] cx-text-faint">{title}</div><p className="mt-2 text-lg font-bold cx-text">{copy}</p></div></div>)}</div></div></section>

        <PricingSection />

        <section id="community" className="px-5 pb-28"><div className="relative mx-auto max-w-7xl overflow-hidden rounded-[32px] border border-white/[0.1] bg-gradient-to-br from-[#0e1830] via-[#111027] to-[#1a0e26] px-6 py-16 text-center sm:px-12 sm:py-24"><div className="absolute left-0 top-0 h-64 w-64 rounded-full bg-cyan-500/10 blur-[90px]"/><div className="absolute bottom-0 right-0 h-64 w-64 rounded-full bg-fuchsia-500/10 blur-[90px]"/><Users className="relative mx-auto h-10 w-10 text-cyan-300"/><h2 className="relative mx-auto mt-6 max-w-3xl text-4xl font-black tracking-[-0.04em] sm:text-6xl">Trade with a system.<br/>Grow with a community.</h2><p className="relative mx-auto mt-5 max-w-xl text-lg cx-text-muted">Share setups, compare process, and improve alongside traders who care about confirmation over prediction.</p><button onClick={() => openAuth('signup')} className="relative mt-8 rounded-2xl bg-white px-7 py-4 font-black text-[#070a12] transition hover:-translate-y-1">Join the platform</button></div></section>
      </main>

      <footer className="relative z-10 border-t cx-border px-5 py-12"><div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-8 sm:flex-row sm:items-center"><ConfluenceXLogo size="md" showTagline/><div className="flex flex-wrap gap-6 text-sm cx-text-faint"><a href="#platform" className="hover:cx-text-strong">Platform</a><a href="#pricing" className="hover:cx-text-strong">Access</a><a href="#community" className="hover:cx-text-strong">Community</a></div><div className="text-xs cx-text-faint">© 2026 ConfluenceX. Market intelligence, converged.</div></div></footer>
