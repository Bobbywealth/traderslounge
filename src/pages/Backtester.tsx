...
const Backtester: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[24px] border border-violet-400/15 cx-bg-card bg-gradient-to-br from-violet-500/10 to-cyan-500/[0.04] p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <div className="text-[10px] font-black tracking-[0.22em] text-cyan-300">DECISION VALIDATION</div>
            <h1 className="mt-2 flex items-center gap-3 text-3xl font-black cx-text-strong"><BarChart3 className="h-7 w-7 text-cyan-300" /> Calibration & Walk-Forward</h1>
            <p className="mt-2 max-w-3xl text-sm cx-text-muted">Outcome evidence, leakage-safe replay, setup slices, expectancy, and adverse excursion. Scenario weights remain uncalibrated until the evidence passes the thresholds below.</p>
          </div>
        ...
        <>
          <section className={`rounded-2xl border p-4 ${report.status === 'CALIBRATED' ? 'border-emerald-400/25 bg-emerald-400/[0.07]' : 'border-amber-400/25 bg-amber-400/[0.07]'}`}>
            <div className="flex items-start gap-3">
              ...
              <div><div className="text-sm font-black cx-text-strong">{report.status.replace(/_/g, ' ')}</div><p className="mt-1 text-xs leading-relaxed cx-text-muted">{report.warning}</p></div>
            </div>
          </section>
          ...
          <section className="rounded-[20px] border cx-border cx-bg-card p-5">
            <div className="flex items-center justify-between"><div><div className="text-[9px] font-black tracking-[0.18em] text-violet-300">RELIABILITY CURVE</div><h2 className="mt-1 text-lg font-black">Weight versus observed outcome</h2></div><span className="rounded-md cx-bg-card-hover px-2 py-1 text-[9px] font-black cx-text-muted">{calibration?.sample_size || 0} samples</span></div>
            <div className="mt-5 grid grid-cols-10 items-end gap-2">
              {bins.map((bin, index) => (
                <div key={index} className="text-center">
                  ...
                  <div className="mt-2 text-[8px] font-black cx-text-faint">{index * 10}-{(index + 1) * 10}</div>
                  <div className="text-[8px] cx-text-faint">n={bin.sample_size}</div>
                </div>
              ))}
            </div>
          </section>
...
const Metric: React.FC<{ label: string; value: string }> = ({ label, value }) => <div className="rounded-xl border cx-border cx-bg-card p-4"><div className="text-[9px] font-black tracking-[0.15em] cx-text-faint">{label}</div><div className="mt-2 text-2xl font-black cx-text-strong">{value}</div></div>;
