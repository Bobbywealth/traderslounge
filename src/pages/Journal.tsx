...
const OUTCOME_TONE: Record<string, string> = {
  ...
  be: 'border-slate-400/20 bg-slate-400/[0.06] text-slate-300',
...
const useSourceLabel = (source: string | undefined | null): { label: string; tone: string; icon: React.ComponentType<{ className?: string }> } => {
  ...
  return { label: source || 'unknown', tone: 'border-slate-400/20 bg-slate-400/[0.06] text-slate-400', icon: AlertTriangle };
};
...
const StatCard: React.FC<StatCardProps> = ({ label, value, detail, tone = 'neutral' }) => (
  <div className="rounded-2xl border border-white/[0.06] bg-[#090d18] p-4">
  ...
  return (
    <button
      ...
      className={`flex w-full items-start gap-3 rounded-2xl border p-4 text-left transition ${
        isActive ? `${meta.tone} ring-1 ring-cyan-400/30` : 'border-white/[0.06] bg-[#090d18] hover:bg-white/[0.03]'
      }`}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <div className="text-base font-black text-white">{count.toLocaleString()}</div>
        </div>
...
const CalibrationBucket: React.FC<CalibrationBucketProps> = ({ label, bucket }) => (
  <div className="rounded-xl border border-white/[0.06] bg-black/20 p-3">
    <div className="flex items-center justify-between text-[10px] uppercase tracking-wider text-slate-500">
      <span>{label}</span>
      <span className="font-mono text-white">{bucket.count.toLocaleString()}</span>
    </div>
    ...
    <div className="mt-2 flex items-center justify-between text-[11px]">
      <span className="text-slate-400">Win {formatPercent(bucket.win_rate * 100, { maximumFractionDigits: 0 })}</span>
...
const Journal: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-10 text-slate-100" data-testid="journal-page">
      ...
      {showForm && (
        <section className="rounded-3xl border border-cyan-400/15 bg-[#090d18] p-6">
          ...
          <form onSubmit={submitDecision} className="mt-4 grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Pair</span>
              <input
                type="text"
                required
                value={draft.pair}
                ...
                placeholder="BTCUSD"
                className="mt-1 w-full rounded-md border border-white/[0.06] bg-black/30 px-3 py-2 text-sm text-white focus:border-cyan-400/30 focus:outline-none"
                data-testid="decision-pair"
              />
            </label>
            <label className="block">
              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Direction</span>
              <select
            ...
            <label className="block">
              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Timeframe</span>
              <select
            ...
            <label className="block">
              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Setup score (0–100)</span>
              <input
                type="number"
                min={0}
                max={100}
            ...
            <label className="block sm:col-span-2">
              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Reasoning</span>
              <textarea
            ...
            <label className="block">
              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">Outcome (optional)</span>
              <input
                type="text"
                value={draft.outcome}
            ...
            <label className="block">
              <span className="text-[10px] font-black uppercase tracking-[0.14em] text-slate-500">R multiple (optional)</span>
              <input
                type="number"
                step="0.1"
            ...
            <div className="sm:col-span-2 mt-2 flex items-center justify-end gap-2">
              <button
                ...
                className="rounded-md border border-white/[0.06] bg-white/[0.04] px-3 py-2 text-xs font-bold text-slate-300 transition hover:bg-white/[0.08]"
              >
                Reset
              </button>
              <button
      ...
      <section className="rounded-3xl border border-white/[0.06] bg-[#090d18] p-5">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="text-sm font-black text-white">Calibration by score bucket</h2>
            <p className="mt-1 text-xs leading-relaxed text-slate-400">
      ...
      {filteredTrades.length > 0 && (
        <section className="overflow-hidden rounded-3xl border border-white/[0.06] bg-[#090d18]">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {filteredTrades.map((t) => {
                  ...
                  const outcomeTone = OUTCOME_TONE[outcome.toLowerCase()] || 'border-slate-400/20 bg-slate-400/[0.06] text-slate-300';
                  ...
                  return (
                    <tr key={t.id} className="border-t border-white/[0.04] hover:bg-white/[0.02]" data-testid="journal-row">
                      <td className="px-4 py-2 font-bold text-white">{t.pair}</td>
                      <td className={`px-4 py-2 ${t.direction === 'BUY' ? 'text-emerald-300' : 'text-rose-300'}`}>
                        {t.direction}
                      </td>
                      <td className="px-4 py-2 text-right font-mono text-slate-300">{formatPrice(t.entry, { maximumFractionDigits: 5 })}</td>
                      <td className="px-4 py-2 text-right font-mono text-slate-300">{formatPrice(t.exit_price, { maximumFractionDigits: 5 })}</td>
                      <td className="px-4 py-2 text-right text-slate-400">{t.lot_size.toFixed(2)}</td>
      ...
      <section className="rounded-3xl border border-violet-400/15 bg-violet-400/[0.04] p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 className="text-sm font-black text-white">The learning loop</h2>
            <p className="mt-1 text-xs leading-relaxed text-slate-400">
            ...
            <ol className="mt-3 space-y-2 text-xs leading-relaxed text-slate-300">
              <li><b className="text-white">Log before you click.</b> Capture the reasoning when you are calm, not after the P&L is known.</li>
              <li><b className="text-white">Compare to the engine.</b> When a decision goes against you, ask whether the engine flagged the same risk at the time.</li>
              <li><b className="text-white">Retire a pattern after three strikes.</b> If the same setup type loses three times across calibration buckets, stop taking it for a quarter.</li>
            </ol>
          </div>
        </div>
      </section>
